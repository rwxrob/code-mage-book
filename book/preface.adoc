...
Rather than get bogged down with a lot of installation and configuration we've created the code-mage command (`cm`) to help you get started as fast as possible without losing the opportunity to learn how to do you own setup later. Eventually, you can come back and examine everything code-mage did to understand it and even customize it. In fact, there's an entire book dedicated to becoming a true terminal master including bash scripting and managing your own "dot files" GitHub repository.
...
- Display local code-mage-book community announcements
